...
-------------------------------------------- */
function getContentBounds(items: CanvasItem[], padding: number = 50) {
  if (!items.length) {
...
function createCroppedStage(
  ...
  const bounds = getContentBounds(items, padding);
...
export async function exportToPDF(
  ...
  const imageBlob = await exportToImage(stage, imageOptions, items);
