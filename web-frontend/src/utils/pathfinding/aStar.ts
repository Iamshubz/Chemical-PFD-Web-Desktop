...
class PriorityQueue<T> {
    ...
    contains(item: T, comparator: (a: T, b: T) => boolean): boolean {
        return this.items.some(({ item: queueItem }) => comparator(item, queueItem));
    }
 ...
 */
function getNeighbors(point: GridPoint, grid: boolean[][], bounds: { width: number; height: number }): GridPoint[] {
...
export function aStar(
    ...
    while (!openSet.isEmpty()) {
        ...
        for (const neighbor of neighbors) {
            ...
            const existingNode = openSet.find(node => pointsEqual(node.point, neighbor));
