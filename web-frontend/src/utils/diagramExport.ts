...
export function validateExportData(data: any): data is DiagramExportData {
  ...
  if (
    !Array.isArray(canvasState.items) ||
    !Array.isArray(canvasState.connections)
  ) {
    return false;
  }
 ...
 */
export async function importFromDiagramFile(
  file: File,
): Promise<DiagramExportData> {
